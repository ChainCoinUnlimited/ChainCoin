...
## `disable-wallet` mode
When the intention is to run only a P2P node without a wallet, Chaincoin Core may be
...
The first time you run chaincoind, it will start downloading the blockchain. This process could
...
## Other commands:
```shell
./src/chaincoind -daemon      # Starts the chainoin daemon.
...
- The Apple keyholder uses this unsigned app to create a detached signature, using the
  script that is also included there. Detached signatures are available from this [repository](https://github.com/chaincoin/chaincoin-detached-sigs).
