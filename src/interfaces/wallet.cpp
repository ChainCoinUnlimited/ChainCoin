...
public:
        ...
        int nCoinJoin) override
    {
        auto locked_chain = m_wallet->chain().lock();
        LOCK(m_wallet->cs_wallet);
        m_wallet->CommitTransaction(std::move(tx), std::move(value_map), std::move(order_form), nCoinJoin);
    }
    ...
    bool tryGetBalances(WalletBalances& balances, CoinJoinStatus& status, int& num_blocks, bool force, int cached_num_blocks) override
    {
        auto locked_chain = m_wallet->chain().lock(true /* try_lock */);
    ...
    int getWalletBackups() override
    {
        return nWalletBackups;
    }
