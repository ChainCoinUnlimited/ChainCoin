...
enum ServiceFlags : uint64_t {
    ...
    // set by all Chaincoin non pruned nodes, and is unset by SPV clients or other light clients.
    NODE_NETWORK = (1 << 0),
    // NODE_GETUTXO means the node is capable of responding to the getutxo protocol request.
    // Chaincoin does not support this but a patch set called Bitcoin XT does.
    // See BIP 64 for details on how this is implemented.
    NODE_GETUTXO = (1 << 1),
    // NODE_BLOOM means the node is capable and willing to handle bloom-filtered connections.
    // Chaincoin nodes used to support this by default, without advertising this bit,
    ...
    // NODE_SERVICE indicates that a node claims to be highly avalable and can be to perform tasks including
    // CoinJoin!.
    NODE_SERVICE = (1 << 30),
