...
void CMainSignals::ProcessModuleMessage(CNode* pfrom, const NetMsgDest& dest, const std::string& strCommand, CDataStream& vRecv) {
    m_internals->Iterate([&](CValidationInterface& callbacks) { callbacks.ProcessModuleMessage(pfrom, dest, strCommand, vRecv); });
}

void CMainSignals::NotifyGovernanceObject(const CGovernanceObject &gobject) {
    m_internals->Iterate([&](CValidationInterface& callbacks) { callbacks.NotifyGovernanceObject(gobject); });
}

void CMainSignals::NotifyGovernanceVote(const CGovernanceVote &govote) {
    m_internals->Iterate([&](CValidationInterface& callbacks) { callbacks.NotifyGovernanceVote(govote); });
}
