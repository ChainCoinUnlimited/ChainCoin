...
//! Check MN Collateral
bool CheckDeposit(const COutPoint& outpointRet, CTxDestination &destRet, CPubKey& pubKeyRet, CKey& keyRet);
...
public:
    ...
    // memory only
    enum AmountType { DEBIT, CREDIT, IMMATURE_CREDIT, AVAILABLE_CREDIT, DENOMINATED_CREDIT, ANONYMIZED_CREDIT, AMOUNTTYPE_ENUM_ELEMENTS };
    ...
    CAmount GetDenominatedCredit(int nCoinJoinDepth=0, bool fUseCache=true, const isminefilter& filter=ISMINE_SPENDABLE) const NO_THREAD_SAFETY_ANALYSIS;
...
public:
    ...
    /// Get keys which can be used for the Masternode
    bool GetMasternodeKeys(const COutPoint& outpoint, CTxDestination &destRet, CPubKey& pubKeyRet, CKey& keyRet);
     ...
     * @param[in] CoinJoin level to be achieved
     */
    void CommitTransaction(CTransactionRef tx, mapValue_t mapValue, std::vector<std::pair<std::string, std::string>> orderForm, int nCoinJoin = 0);
