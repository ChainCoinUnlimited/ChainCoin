...
{
    // If wallet doesn't have a chain (e.g chaincoin-wallet), lock can't be taken.
...
CAmount CWalletTx::GetDenominatedCredit(int nCoinJoinDepth, bool fUseCache, const isminefilter& filter) const
{
    if (pwallet == nullptr)
        return 0;
    ...
    for (unsigned int i = 0; i < tx->vout.size(); i++)
    {
        if(!pwallet->IsSpent(hashTx, i) && CCoinJoin::IsDenominatedAmount(tx->vout[i].nValue))
        {
            const CTxOut &txout = tx->vout[i];
...
CAmount CWallet::GetLegacyBalance(const isminefilter& filter, int minDepth) const
{
    ...
    for (const auto& entry : mapWallet) {
        ...
        if (depth < 0 || !locked_chain->checkFinalTx(*wtx.tx) || wtx.IsImmatureCoinBase()) {
            continue;
        }
...
void CWallet::CommitTransaction(CTransactionRef tx, mapValue_t mapValue, std::vector<std::pair<std::string, std::string>> orderForm, int nCoinJoin)
{
    auto locked_chain = chain().lock();
    LOCK(cs_wallet);
...
DBErrors CWallet::LoadWallet(bool& fFirstRunRet)
{
    ...
    // tx status. If lock can't be taken (e.g chaincoin-wallet), tx confirmation
...
bool CWallet::Verify(interfaces::Chain& chain, const WalletLocation& location, bool salvage_wallet, std::string& error_string, std::vector<std::string>& warnings)
{
    ...
    if (!WalletBatch::VerifyDatabaseFile(wallet_path, warnings, error_string)) {
        return false;
    }

    return AutoBackupWallet(nullptr, location, warnings, error_string);
}
    ...
    if (pwallet)
    {
        ...
        if(fs::exists(sourceFile)) {
            try {
                ...
                warnings.push_back(strprintf(_("Failed to create backup, error: %s").translated, error.what()));
                LogPrintf("%s\n", Join(warnings, "\n"));
                nWalletBackups = -1;
                return false;
            }
    ...
    for (std::pair<const time_t, fs::path> file : reverse_iterate(folder_set))
    {
        ...
        if (counter > nWalletBackups)
        {
            ...
            try {
                ...
                warnings.push_back(strprintf(_("Failed to delete backup, error: %s").translated, error.what()));
                LogPrintf("%s\n", Join(warnings, "\n"));
                return false;
            }
        }
