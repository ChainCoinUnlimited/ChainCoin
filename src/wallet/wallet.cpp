...
bool CWallet::Unlock(const SecureString& strWalletPassphrase, bool accept_no_keys, bool fForMixingOnly)
{
...

    {
        ...
        for (const MasterKeyMap::value_type& pMasterKey : mapMasterKeys)
        {
            ...
            if (Unlock(_vMasterKey, accept_no_keys, fForMixingOnly)) {
                if(nWalletBackups == -2) {
                    TopUpKeyPool();
...
CAmount CWalletTx::GetDenominatedCredit(interfaces::Chain::Lock& locked_chain, int nCoinJoinDepth, bool fUseCache, const isminefilter& filter) const
{
    if (pwallet == nullptr)
        return 0;
    ...
    if (fUseCache && allow_cache && m_amounts[DENOMINATED_CREDIT].m_cached[filter]) {
        return m_amounts[DENOMINATED_CREDIT].m_value[filter];
    }
    ...
    if (nCoinJoinDepth >= pwallet->coinjoinClient->nCoinJoinDepth && allow_cache) {
        m_amounts[ANONYMIZED_CREDIT].Set(filter, nCredit);
    } else if (allow_cache) {
        m_amounts[DENOMINATED_CREDIT].Set(filter, nCredit);
    }
...
void CWallet::AvailableCoins(interfaces::Chain::Lock& locked_chain, std::vector<COutput>& vCoins, bool fOnlySafe, const CCoinControl* coinControl, AvailableCoinsType nCoinType, const CAmount& nMinimumAmount, const CAmount& nMaximumAmount, const CAmount& nMinimumSumAmount, const uint64_t nMaximumCount) const
{
    AssertLockHeld(cs_wallet);
...
bool CWallet::SelectJoinCoins(CAmount nValueMin, CAmount nValueMax, std::vector<std::pair<CTxIn, CTxOut> >& cjPairRet, int nMinDepth) const
{
    ...
    LOCK(cs_wallet);
    AvailableCoins(*locked_chain, vCoins, true, &cctl, ONLY_DENOMINATED, 1, MAX_MONEY, MAX_MONEY);
...
bool CWallet::GetBudgetSystemCollateralTX(interfaces::Chain::Lock& locked_chain, CTransactionRef& tx, uint256 hash, CAmount amount)
{
    ...
    bool success = CreateTransaction(locked_chain, vecSend, tx, nFeeRet, nChangePosRet, strFail, coin_control, true, ALL_COINS);
    if(!success){
                         ...
                         int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign, AvailableCoinsType _nCoinType, int nCoinJoin)
{
    ...
    int nBytes;
    {
        ...
        LOCK(cs_wallet);
        {
            ...
            AvailableCoins(*locked_chain, vAvailableCoins, true, &coin_control, nCoinType, 1, MAX_MONEY, MAX_MONEY, 0);
                    ...
                    if (!SelectCoins(vAvailableCoins, nValueToSelect, setCoins, nValueIn, coin_control, coin_selection_params, bnb_used))
                    {
                        ...
                        if (bnb_used) {
                            ...
                            if (nCoinType == ONLY_NONDENOMINATED)
                                strFailReason = _("Unable to locate enough CoinJoin non-denominated funds for this transaction that are not equal 1000 CHC.").translated;
                            if (nCoinType == ONLY_DENOMINATED)
                                strFailReason = _("Unable to locate enough CoinJoin denominated funds for this transaction.").translated;
                                strFailReason += " " + _("CoinJoin uses exact denominated amounts to send funds, you might simply need to anonymize some more coins.").translated;
 ...
 */
bool CWallet::CommitTransaction(CTransactionRef tx, mapValue_t mapValue, std::vector<std::pair<std::string, std::string>> orderForm, CValidationState& state, int nCoinJoin)
{
    {
        auto locked_chain = chain().lock();
        LOCK(cs_wallet);
...
bool AutoBackupWallet (std::shared_ptr<CWallet> pwallet, const WalletLocation& location, std::string& strBackupWarning, std::string& strBackupError)
{
    ...
    if (!fs::is_directory(backupsDir)) {
        ...
        strBackupError = strprintf(_("%s is not a valid backup folder!").translated, backupsDir.string());
        LogPrintf("%s\n", strBackupError);
        nWalletBackups = -1;
        return false;
    }
    ...
    if (pwallet)
    {
        ...
        if(!pwallet->BackupWallet(backupFile.string())) {
            strBackupWarning = strprintf(_("Failed to create backup %s!").translated, backupFile.string());
        ...
        if(pwallet->IsLocked(true)) {
            strBackupWarning = _("Wallet is locked, can't replenish keypool! Automatic backups and mixing are disabled, please unlock your wallet to replenish keypool.").translated;
            pwallet->WalletLogPrintf("%s\n", strBackupWarning);
            nWalletBackups = -2;
            return false;
        }
        ...
        {
            strBackupWarning = _("Failed to create backup, file already exists! This could happen if you restarted wallet in less than 60 seconds. You can continue if you are ok with this.").translated;
            LogPrintf("%s\n", strBackupWarning);
            return false;
        }
        if(fs::exists(sourceFile)) {
            try {
                ...
                strBackupWarning = strprintf(_("Failed to create backup, error: %s").translated, error.what());
                LogPrintf("%s\n", strBackupWarning);
                nWalletBackups = -1;
                return false;
            }
    ...
    for (std::pair<const time_t, fs::path> file : reverse_iterate(folder_set))
    {
        ...
        if (counter > nWalletBackups)
        {
            ...
            try {
                ...
                strBackupWarning = strprintf(_("Failed to delete backup, error: %s").translated, error.what());
                LogPrintf("%s\n", strBackupWarning);
                return false;
            }
        }
