...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-conf=<file>", strprintf("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)", CHAINCOIN_CONF_FILENAME), ArgsManager::ALLOW_ANY, false, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", CHAINCOIN_PID_FILENAME), ArgsManager::ALLOW_ANY, false, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-zmqpubhashgovernancevote=<address>", "Enable publish hash of funding votes in <address>", ArgsManager::ALLOW_ANY, false, OptionsCategory::ZMQ);
    gArgs.AddArg("-zmqpubhashgovernanceobject=<address>", "Enable publish hash of funding objects (like proposals) in <address>", ArgsManager::ALLOW_ANY, false, OptionsCategory::ZMQ);
    ...
    gArgs.AddArg("-zmqpubhashgovernancevotehwm=<n>", strprintf("Set publish hash of funding votes message high water mark (default: %d)", CZMQAbstractNotifier::DEFAULT_ZMQ_SNDHWM), ArgsManager::ALLOW_ANY, false, OptionsCategory::ZMQ);
    gArgs.AddArg("-zmqpubhashgovernanceobjecthwm=<n>", strprintf("Set publish hash of funding objects (like proposals) message high water mark (default: %d)", CZMQAbstractNotifier::DEFAULT_ZMQ_SNDHWM), ArgsManager::ALLOW_ANY, false, OptionsCategory::ZMQ);
#else
    ...
    gArgs.AddArg("-litemode=<n>", strprintf(_("Disable all Chaincoin specific functionality (Masternodes, CoinJoin, Funding) (0-1, default: %u)").translated, 0), ArgsManager::ALLOW_ANY, false, OptionsCategory::OPTIONS);

    gArgs.AddArg("-masternode=<n>", strprintf(_("Enable the client to act as a masternode (0-1, default: %u)").translated, 0), ArgsManager::ALLOW_ANY, false, OptionsCategory::OPTIONS);
    gArgs.AddArg("-mnconf=<file>", strprintf(_("Specify masternode configuration file (default: %s)").translated, "masternode.conf"), ArgsManager::ALLOW_ANY, false, OptionsCategory::OPTIONS);
    gArgs.AddArg("-mnconflock=<n>", strprintf(_("Lock masternodes from masternode configuration file (default: %u)").translated, 1), ArgsManager::ALLOW_ANY, false, OptionsCategory::OPTIONS);
    gArgs.AddArg("-masternodeprivkey=<n>", _("Set the masternode private key").translated, ArgsManager::ALLOW_ANY, false, OptionsCategory::OPTIONS);
...
std::string LicenseInfo()
{
    ...
    return CopyrightHolders(strprintf(_("Copyright (C) %i-%i").translated, 2009, COPYRIGHT_YEAR) + " ") + "\n" +
           ...
           _("Distributed under the MIT software license, see the accompanying file COPYING or <http://www.opensource.org/licenses/mit-license.php>.").translated + "\n" +
           "\n" +
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    if(fLiteMode) {
        InitWarning(_("Starting in lite mode, all Chaincoin-specific functionality is disabled.").translated);
    }

    if(fLiteMode && fMasternodeMode) {
        return InitError(_("Masternodes are not supported in lite mode.").translated);
    }
    ...
    if (!fLiteMode) {
        ...
        if(mnodeman.size()) {
            ...
            uiInterface.InitMessage(_("Masternode cache is empty, skipping payments and funding cache...").translated);
        }
        CNetFulDB netfuldb;
        ...
        uiInterface.InitMessage(_("Loading CoinJoin! cache...").translated);
        CCoinJoinDB coinjoindb;
