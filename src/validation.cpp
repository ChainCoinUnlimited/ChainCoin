                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.Invalid(BlockValidationResult::CONSENSUS,
                            error("ConnectBlock(CHAINCOIN): %s", strError),
                            "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(MakeTransactionRef(std::move(*block.vtx[0])), pindex->nHeight, blockReward)) {
        return state.Invalid(BlockValidationResult::CONSENSUS,
                            error("ConnectBlock(CHAINCOIN): couldn't find masternode or superblock payments"),
                            "bad-cb-payee");
    }
    // END CHAINCOIN
